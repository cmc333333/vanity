...
const DrupalsServiceModule: React.FC = () => (
  <Layout sidebar={writingsSidebar} title="Drupal's Services Module">
    <p>
      ...
      <a href="http://drupal.org/project/services">Services</a> module, which
    ...
    <p>
      ...
      <a href="http://groups.drupal.org/services">Services Group</a> which can
    ...
    <p>
      ...
      &ldquo;Servers,&rdquo; represent entry points (i.e. protocols) to your
      ...
      lots of library support, and it&rsquo;s very simple to model.
    </p>
    <p>
      The other category, &ldquo;Services,&rdquo; represent the functionality
      which you&rsquo;d like to make accessible to others. For example, the
      Services project comes with the Node service, which allows you to get,
      save, and delete nodes from afar. We&rsquo;ll discuss this service as well
    ...
    <p>
      ...
      services-specific framework, they chose to reuse Drupal&rsquo;s core user
      ...
      these nodes, etc. is limited based on that user&rsquo;s permissions. This
      has several implications on how you&rsquo;ll need to set up user
    ...
    <p>
      ...
      is to use a more Unix-style &ldquo;daemon&rdquo; user for each service. In
      this way <em>all</em> services that need to search (for example) act as
    ...
    <p>
      ...
      installations of the Services module, I&rsquo;ve needed to rebuild user
      ...
      &gt;Post Settings. I&rsquo;m not too sure what the problem is, nor if it
    ...
    <p>
      ...
      Services. You&rsquo;ll first notice the API Key management tool which
      ...
      admin, though I don&rsquo;t see any reason why you couldn&rsquo;t develop
      ...
      using it. I believe the <em>purpose</em> of the domain was to restrict key
      access to only certain IPs/domains, but this doesn&rsquo;t appear to be
      implemented at this time. In fact, as we will see in a moment, the service
      receiver simply provides the &ldquo;domain&rdquo; string as a parameter
    ...
    <p>
      ...
      also be able to retrieve this information from the &ldquo;system&rdquo;
    ...
    <p>
      It&rsquo;s important to note that viewing/testing these services requires
      ...
      than rewriting into &ldquo;clean URLs&rdquo;). If you can&rsquo;t view the
      documentation, these web server rules are probably to blame. For example,
      in my lighttpd configuration, I&rsquo;ve added
    </p>
    <pre>
      <code>
