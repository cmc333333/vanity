/*
 * Presentation Plugin  + modifications
 * http://www.viget.com/
 *
  ...
  $.fn.presentation = function(options) {
    ...
    return $(this).map(function() {
      ...
      $presentation.changeSlide = function(newSlide) {
        ...
        $presentation.slideToShow = $presentation.slides.slice(newSlide-1, newSlide);
      ...
      $presentation.prevNextClick = function(action) {
        if(action === 'prev') {
          ...
          var altPath = false;
          for (index in $presentation.options.altPath) {
            if ($presentation.options.altPath[index][0] == $presentation.count) {
              $presentation.count = $presentation.options.altPath[index][1];
              altPath = true;
              break;
            }
          }
      ...
      $presentation.init = function() {
        ...
        if($presentation.currentHash) {
          ...
          $presentation.slides.filter(':first').show();
...
jQuery("#enter-presentation").click(function() {
  jQuery(this).hide();
  jQuery("div.detail").hide();
  var presentation = jQuery("#presentation").presentation({
    transition: 'show/hide', 
    altPath: [
      [8,1], [11,1], [13,1], [14,1], [27,1], [29,1], [31,1], [33,1]
    ]
  });
  jQuery("#topics a").click(function() {
    jThis = jQuery(this);
    var id = parseInt(jThis.attr("href").substring(1));
    presentation[0].count = id;
    presentation[0].changeSlide(id);
    jThis.css("text-decoration", "line-through");
  });
});
