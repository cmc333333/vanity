...
const fetchPoster = async (
  ...
  traktType,
  ids,
  { actions: { createNode }, cache, createNodeId, reporter, store },
  { mdataAuth }
) => {
...
exports.sourceNodes = async (sourceNodeArgs, pluginArgs) => {
  ...
  const movieHistory = await client.users.history({
    limit: 10000,
    type: "movies",
    username: "me",
  });
  const episodeHistory = await client.users.history({
    limit: 10000,
    type: "shows",
    username: "me",
  });
  const favoritesList = await client.users.list.items.get({
    username: "me",
    id: 10331465,
  });
